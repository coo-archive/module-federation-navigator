...
function toRoute<TModuleConfig extends ngcx.module.federation.navigation.NavigableRemoteModuleConfig>(
...
function toComponentDefinition(
	_definition: ngcx.module.federation.navigation.RemoteComponentEsmOptions | ngcx.HTML
): Promise<ComponentType<unknown> | ngcx.HTML> {
	switch (typeof _definition) {
		case 'string': {
			return new Promise<ngcx.HTML>(resolve => {
				resolve(_definition);
			});
		}
		default: {
...
function toNavigationDefinition<TEvent>(
	_definition: ngcx.module.federation.navigation.item.Definition,
	_injector: Injector
): ngcx.navigation.item.Definition<TEvent> {
	let _options: Partial<ngcx.navigation.item.Options<TEvent>>;
	try {
...
export function buildModuleNavigatorDefinition<TModuleConfig extends ngcx.module.federation.navigation.NavigableRemoteModuleConfig>(
	cfg: ngcx.module.federation.navigation.navigator.Manifest<TModuleConfig>,
	_injector: Injector
): ngcx.module.federation.navigation.navigator.Definition {
	const modules = Object
		.keys(cfg)
		.map<ngcx.module.federation.navigation.NavigableRemoteModuleConfig>(_section => cfg[_section])
	...
	return ({
		...
		navigation: modules
			...
			.map((_definition: ngcx.module.federation.navigation.item.Definition) => toNavigationDefinition<unknown>(_definition, _injector))

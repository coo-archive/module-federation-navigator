...
export class ModuleNavigatorManifest<TModuleConfig extends ngcx.module.federation.navigation.NavigableRemoteModuleConfig> {

	private readonly _definition$ = new ReplaySubject<ngcx.module.federation.navigation.navigator.Definition>();

	public readonly definition$: Observable<ngcx.module.federation.navigation.navigator.Definition> = this._definition$;
	...
	public load(file: string): Promise<void> {
		return fetch(file).then((result) => {
			...
			return result.json() as Promise<ngcx.module.federation.navigation.navigator.Manifest<TModuleConfig>>;
		}).then(_cfg => {
